...
import { Wrench, Pencil, Trash2, CalendarCheck } from 'lucide-react';
import type { PageProps } from '@/types';

interface Option { value: string; label: string }
interface ExecutionRecord {
    id: string; done_date: string; cost: string | null; notes: string | null;
    supplier: { id: string; name: string } | null;
    author: { id: string; name: string } | null;
}
interface Plan {
    id: string; title: string; description: string | null; category: string | null; frequency: string;
    next_due_date: string | null; last_done_date: string | null; alert_days: number; is_active: boolean;
    status: string | null; days_until_due: number | null;
...
function fmtMoney(v: string | null) {
    return v !== null && v !== '' ? Number(v).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : '—';
}

export default function MaintenanceShow({ plan, categories, frequencies, suppliers }: Props) {
    const { auth } = usePage<PageProps>().props;
    const perms = auth.user?.permissions ?? [];
    const can = (p: string) => perms.includes('*') || perms.includes(p);

    const today = new Date().toISOString().slice(0, 10);
    const form = useForm({ done_date: today, supplier_id: plan.supplier?.id ?? '', cost: '', notes: '' });
    const submit = () => form.post(route('maintenance.executions.store', plan.id), {
        preserveScroll: true,
        onSuccess: () => form.reset('cost', 'notes'),
    });
    const removeRecord = (id: string) => {
        if (confirm('Remover esta execução?')) router.delete(route('maintenance.executions.destroy', id), { preserveScroll: true });
    };
    ...
    return (
        <AppLayout>
            ...
            <div className="mx-auto max-w-3xl space-y-6">
                ...
                <div className="space-y-2 rounded-xl border border-gray-100 bg-white p-6 shadow-sm text-sm text-gray-700">
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {plan.records.length === 0 && (
                                    <tr><td colSpan={5} className="px-3 py-6 text-center text-sm text-gray-500">Nenhuma execução registrada.</td></tr>
                                )}
                                {plan.records.map(r => (
                                    <tr key={r.id}>
                                        <td className="px-3 py-2 text-gray-700">{fmtDate(r.done_date)}</td>
                                        <td className="px-3 py-2 text-xs text-gray-600">{r.supplier?.name ?? '—'}</td>
                                        <td className="px-3 py-2 text-xs text-gray-600">{fmtMoney(r.cost)}</td>
                                        <td className="px-3 py-2 text-xs text-gray-600">{r.notes ?? '—'}</td>
                                        <td className="px-3 py-2 text-right">
                                            {can('maintenance:delete') && (
                                                <button onClick={() => removeRecord(r.id)} className="rounded p-1.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
